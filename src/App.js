  ...
  const fetchTickets = async () => {
    try {
      ...
      if (data.tickets && data.users) {
        ...
        // Map priority to human-readable names
        ...
        const mappedTickets = data.tickets.map(ticket => {
          ...
          const priorityObj = priorityMapping.find(p => p.priority === ticket.priority) || priorityMapping[0];
          return { ...ticket, userObj, priorityName: priorityObj.name };
        });

        setTicketDetails(mappedTickets);

        // Prepare priority list
        setPriorityList(priorityMapping);
      } else {
        console.error('Missing data structure in response:', data);
      }
    } catch (error) {
