...
export default function List(props) {
    ...
    // Compute the card count locally
    const filteredCards = props.ticketDetails.filter(ticket => {
        ...
        } else if (props.groupValue === 'user' && ticket.userObj) {
            return ticket.userObj.name === props.listTitle;
        }
        return false;
    });

    const cardCount = filteredCards.length; // Get the total number of relevant cards
        ...
        <>
            <div className="list-container">
                            ...
                            {
                                {
                                    'priority': props.priorityList
                                        ? props.priorityList.find(
                                              priorityProperty => priorityProperty.priority === props.listTitle
                                          )?.name || props.listTitle
                                        : props.listTitle,
                ...
                {filteredCards.map(ticket => (
                        <Card key={ticket.id || ticket.title} cardDetails={ticket} />
                    ))}
                </div>
            </div>
        </>
